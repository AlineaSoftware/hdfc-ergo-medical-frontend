...
import { editAutoEmpanelment } from '@/lib/autoEmpanelment'
import CheckInput from '@/components/CheckInput'

type Props = {}

const AutoEditList = (props: Props) => {
  ...
  const getData = async () => {
    const response = await getOneEmpanelmentData(setLoading, showToast, lastSegment)
    if (response) {
  ...
  const onSubmitHandle: SubmitHandler<any> = async (data) => {
    ...
    const res = await editAutoEmpanelment(setLoading, showToast, newData)
    if (res?.success) {
      nav('/thankyou')
    }
  }
      ...
      <div className='mt-3 mb-8'>
        <div className='flex items-center py-2 w-full px-4 bg-white-main shadow rounded-md justify-between gap-5 mb-3'>
          <div className='flex gap-10'>
            <span className='text-base font-semibold italic'>Edit Self Empanelment</span>
          </div>
        </div>
        {!data?.isEditable && (
                      ...
                      <React.Fragment key={x._id || i}>
                        <div className='flex items-end gap-3'>
                          ...
                          <TxtInput
                            ...
                            isDisabled={!data?.isEditable || !watch(`staff.${i}.isAvailable`)}
                          />
                        </div>
                        <SelectInput
                          clearErrors={clearErrors}
                          ...
                          isDisable={!data?.isEditable || !watch(`staff.${i}.isAvailable`)}
                        />
                        <TxtInput
                          control={control}
