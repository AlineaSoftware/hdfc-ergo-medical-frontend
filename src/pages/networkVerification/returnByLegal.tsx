...
const ReturnByLegalList = ({ handleOpen, setType, open, type, handleClose, isDefault }: Props) => {
  ...
  const getData = async () => {
    const response = await getList(setLoading, showToast, setNotFound, notFound, {
      ...handleControls,
      status: EVerificationProcess.ReturnedByLegal,
    })
    if (response) {
      const { records, ...rest } = response
      if (records?.length === 0) {
        setNotFound([TABLES.RETURN_BY_LEGAL])
      } else {
        setNotFound([])
        setData(records)
        setControls(rest)
  ...
  return (
    <Box>
      <Table
        ...
        tableHeading={{
          tableId: TABLES.RETURN_BY_LEGAL,
          tableName: 'Return By Legal',
        }}
        notFound={notFound.includes(TABLES.RETURN_BY_LEGAL)}
        btnTxtArray={[]}
...
export default ReturnByLegalList
