...
import {
  ...
  verifyEmpanelmentByLegal,
} from '@/lib/empanelmentCountAndList'
import CheckInput from '@/components/CheckInput'

type Props = {}

const LegalPendingVerifyDetailsList = (props: Props) => {
  ...
  const onSubmitHandle: SubmitHandler<any> = async (data) => {
    ...
    const res = await verifyEmpanelmentByLegal(setLoading, showToast, {
      remark: remark,
      status: EVerificationProcess?.Verified,
      id: state?.state?._id,
    })
    if (res?.success) {
      nav('/legal-verification')
      // reset()
      // getData()
    }
  }
...
export default LegalPendingVerifyDetailsList
